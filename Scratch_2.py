...
class BridgeManagementApp:
    ...
    def insert_dict_into_table(self, dictionary, textspan = 60):
        """Inserts the dictionary values into the ScrolledText widget in a table format."""
        # Clear the current content
        self.log_listbox.delete(1.0, "end")

        # Insert headers
        self.log_listbox.insert("end", f"{'Key':<{textspan}}{'Value'}\n")
        self.log_listbox.insert("end", "-"*35 + "\n")  # Separator line
    ...
    def create_labels_and_entries(self, frame, labels, remarks, var_types, widgets, Information = ""):
            ...
            def validate_numeric_input(event, var_type, entry_var, entry_widget):
                ...
                try:
                    ...
                    entry_widget.insert(0, "Not valid datatype")  # Reset to default value
            def on_entry_click(event, entry):
            ...
            def scroll_remark(entry, speed=150):
                ...
                def scroll():
                    while True:
                        text_var = entry.get()
                        if len(text_var) > 1:  # Only scroll if text is long enough
            ...
            for i, (text, remark, var_type, widget_type) in enumerate(zip(labels, remarks, var_types, widgets)):
                ...
                if widget_type == "entry":
                    ...
                    options = ["Option 1", "Option 2", "Option 3"]  # Example dropdown options
                    ...
                    entries.append(var)  # Store the dropdown variable
    ...
    def procedural_based_design(self):
        ...
        # Function to create labels and entries based on the provided lists
        ...
        input_labels = ["Bridge span", "Foundation setback - Right", "Foundation setback - Left",
                        ...
                        "Windguy cable elevation downstream - Right", "Windguy cable elevation downstream - Left"]
        input_remarks = ["Horizontal component of bridge span proposed during survey. ",
                         "Foundation shift from proposed position. ",
                         "Foundation shift from proposed position. ", "", "", "", "", "", ""]
        input_var_types = ["float", "float", "float", "float", "float", "float", "float", "float", "float"]
        input_widgets = ["entry", "entry", "entry", "entry", "entry", "entry", "entry", "entry",
                         "entry"]  # Specify widget types
